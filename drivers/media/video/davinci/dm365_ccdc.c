...
{
	printk(KERN_ERR "OUT TO SDRAM: %d", en);
...
static void ccdc_config_culling(struct ccdc_cul *cul)
{
	...
	printk(KERN_ERR "CULLING :%d %d %d %d\n", cul->hcpat_even, cul->hcpat_odd, cul->vcpat, cul->en_lpf);
			...
			enum ccdc_frmfmt frm_fmt, int ppc, int mode)
{
	...
	if (frm_fmt == CCDC_FRMFMT_INTERLACED) {
		printk(KERN_ERR "AAAA");
		vert_nr_lines = (image_win->height >> 1) - 1;
		vert_start >>= 1;
		/* To account for VD since line 0 doesn't have any data */
		vert_start += 1;
	} else {
		printk(KERN_ERR "BBBB");
		/* To account for VD since line 0 doesn't have any data */
		vert_start += 1;
	...
	printk(KERN_ERR "SET WIN XX: %d %d\n", horz_nr_pixels, vert_nr_lines);
}
...
{
	printk(KERN_ERR "CCDC CONDIF RAW \n");
	...
	if (params->frm_fmt == CCDC_FRMFMT_INTERLACED) {
		printk(KERN_ERR "CCDC INTERLACED\n");
		...
		printk(KERN_ERR "CCDC PROGRESSIVE\n");
		if (params->image_invert_en) {
...
{
	printk(KERN_ERR "CCDC SET IMAGE WINDOW: %d %d %d %d", win->top, win->left,
...
static unsigned int ccdc_get_line_length(void)
{
	...
	printk(KERN_ERR "Get line lenght\n");
	printk(KERN_ERR "pack %d width %d height %d bayer%d\n", ccdc_cfg.data_pack,
		ccdc_cfg.bayer.win.width, ccdc_cfg.bayer.win.height, ccdc_cfg.if_type);
...
{
	printk(KERN_ERR "KERN PARAMETERS");
