...
static int vpfe_get_camera_frame_params(struct vpfe_device *vpfe_dev)
{
	...
	sd_fmt.fmt.pix.pixelformat = 	V4L2_PIX_FMT_SBGGR16;
				 ...
				 struct v4l2_format *f)
{
	...
	printk(KERN_ERR "CCDC SETTINGS: %d %d %d\n", image_win.width, image_win.height, ccdc_dev->hw_ops.get_line_length());

	if (frm_fmt == CCDC_FRMFMT_PROGRESSIVE) {
		f->fmt.pix.field = V4L2_FIELD_NONE;
		printk(KERN_ERR "PROGRESSIVE");
...
{
	printk(KERN_ERR "Set format ing sensor\n");
...
{
	printk(KERN_ERR "vpfe_vdint1_isr");
			     ...
			     struct v4l2_crop *crop)
{
	...
	printk(KERN_ERR "STD_CROP: %d %d \n", vpfe_dev->std_info.active_pixels,
		vpfe_dev->std_info.active_lines);

		printk(KERN_ERR "STD_CROP: %d %d \n", vpfe_dev->fmt.fmt.pix.width,
			vpfe_dev->fmt.fmt.pix.height);
