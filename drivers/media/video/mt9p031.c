...
/*Minimum values*/
#define MT9P031_VERTICAL_BLANK_MINIMUM     (0x0008)
    ...
    struct mt9p031 *mt9p031)
{
    ...
    /* Shutter overhead */
    so = 208 * (mt9p031->ybin) + 98 + min((int)shutter_delay, 1504) - 94;
    /* Output image width */
    ...
    hbmin = hb_min[mt9p031->ybin-1][mt9p031->xbin-1];
    /* Row time calculation, fixed point 24:8 in us */
    hb_max = max(hb, hbmin);
    max = max((w/2 + hb_max),(41 + 346*(mt9p031->ybin) + 99));
...
static int get_frame_interval(int width, int height){
    ...
    int index = 0;
    int fps = 0;
    while (index < mt9p031_num_frmsizes) {
        sizei.width = mt9p031_frame_table[index].framesize.width;
        sizei.height = mt9p031_frame_table[index].framesize.height;
    ...
    fps = mt9p031_frame_table[index].frameintervals[0].denominator /
        mt9p031_frame_table[index].frameintervals[0].numerator;
    return fps;
}
                  ...
                  struct v4l2_rect *rect, u16 xskip, u16 yskip)
{
    ...
    if (ret >= 0) {
        ...
        height = 2*((height + mt9p031->y_skip_top)/(2*yskip) + 1) ;
        /* update the values */
        mt9p031->width  = rect->width,
        mt9p031->height = height,
                   ...
                   struct v4l2_control *ctrl)
{
    ...
    case V4L2_CID_EXPOSURE:
        ...
        if (ctrl->value > qctrl->maximum ||
            ...
            if (sw < 1)
                sw = 1;
            vbmin = sw - mt9p031->height + 1;
            if (vbmin < 8) {
                vbmin = mt9p031->vblank;
            }
            if (mt9p031->vblank < (unsigned int)vbmin){
                sw = mt9p031->vblank + mt9p031->height - 1;
            }
            ...
            if (ret < 0){
                ...
                if (mt9p031->vblank < (unsigned int)vbmin){
                    mt9p031->exposure = sw * mt9p031->exp.row_time -
                        (mt9p031->exp.shutter_overlay * 2 *
                        mt9p031->exp.pix_clock)/1000;
                    v4l2_dbg(1, debug, sd,
